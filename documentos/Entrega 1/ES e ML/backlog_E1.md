...
| ID | Frente | Responsável | Estado | Critério de pronto e evidência | Próxima ação / bloqueio |
|---|---|---|---|---|---|
| DAD-01 | Dados | Lúcio (correção); Gustavo (revisão) | Em revisão | `src/prepara.py` reprova ausência de passivo em ano obrigatório; a referência passou localmente e no Colab, com 15/15 verificações e SHA-256 preservado. | Revisão independente do diff e dos resultados por Gustavo. |
| DAD-02 | Dados | Gustavo | Em revisão | Execução técnica integral dos dois notebooks em sessões separadas do Colab em 24/09/2026, commit `3ab2a1e`; versões, resultados e SHA-256 estão registrados no README sem publicar linhas da base. | Revisão independente de Lúcio e confirmação do responsável Gustavo. |
| DAD-03 | Dados | Lúcio (texto); Gustavo (revisão) | Em revisão | DOCX e PDF de Dados foram sincronizados com a execução real no Colab; as afirmações sobre CV, EBITDA e lucro têm evidência. | Revisão independente por Gustavo e conferência dos arquivos publicados. |
...
Registrar neste arquivo ou em um item vinculado: ID afetado, motivo, data, impacto nos artefatos, responsável e revisor. Atualizar os documentos, conferir a saída e associar o commit. Fórmulas que dependem de esclarecimento da CTI ou do professor permanecem pendentes até resposta. Uma mudança no código sem revisão correspondente não conclui o item.

## Evidências e limites de acesso

- Dados: `src/prepara.py`, `src/notebooks/01_perfil.ipynb`, `src/notebooks/02_preparacao.ipynb` e relatório na pasta de Ciência de Dados.
